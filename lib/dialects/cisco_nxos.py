...
def rule_to_acl_ipv4_string(self):
    ...
    for f in filter_to_acl_ipv4_string(self.filter):
        acl.append(' '.join(map(str, [self.action, f, ext])).strip())
    return acl
...
def install(self, timestamp=True):
    ...
    for v in self.context.ip_versions:
        for d in metacl.DIRECTIONS:
            ...
            header, newacl_rules[v][d], footer = compile_one(self, name='TEMP', direction=d,
                                                             ip_version=v, timestamp=timestamp)
...
class Router(generic.Router):
    ...
    def _connect(self, hostname, username, password, port=22, read_running_config_first=False,
                 name=None):
        ...
        *ignore* may contain a list of error messages that will be ignored.
        ...
        try:
            ...
            if not self._client.prompt(timeout=300):
                raise ErrorTimeout("Execution of comamnd and input '%s\n%s' timeouted after 5 " +
            ...
            raise Error("SSH timeout while executing: '%s' on %s" %
                        (command, self.name))
    ...
    def get_bound_acl_form_interface(self, interface):
        ...
        if self._running:
            ...
            assert len(response), "Got empty interface configuration for %s on %s, that can " + \
                "not be right..." % (interface, self.name)
    ...
    def delete(self, path):
        ...
        try:
            ...
            if i == 2:  # Do you want to delete "..." ? (yes/no/abort)   [y]
                ...
                i = self._client.expect([pxssh.TIMEOUT, self._client.PROMPT], timeout=300)
