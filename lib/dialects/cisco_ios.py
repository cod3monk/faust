...
def replace_ianaports(l):
    ...
    for oldportfilter in re.findall(
            r'(?:lt|eq) [a-z0-9\-\+\*]+|range (?:[a-z0-9\-\+\*]+ [a-z0-9\-\+\*]+|\d+ ' +
            r'[a-z0-9\-\+\*]+|[a-z0-9\-\+\*]+ \d)', l):
...
def compile_all(acl, name, timestamp=True):
    '''Takes a :class:`faust2.lib.metacl.ACL` object and returns an cisco ACL string'''

    if acl.context is None:
        raise Error('MetaACL object needs context, to be compiled to an Cisco ACL')

    acl_string = []

    acl_string += map(lambda x: '\n'.join(x),
                      compile_one(acl, name, 'in', 'ipv4', timestamp=timestamp, comments=True))
    acl_string += map(lambda x: '\n'.join(x),
                      compile_one(acl, name, 'out', 'ipv4', timestamp=timestamp, comments=True))

    if 'ipv6' in acl.context.ip_versions:
        acl_string += map(lambda x: '\n'.join(x),
                          compile_one(acl, name, 'in', 'ipv6', timestamp=timestamp, comments=True))
        acl_string += map(lambda x: '\n'.join(x),
                          compile_one(acl, name, 'out', 'ipv6', timestamp=timestamp, comments=True))
...
def install(self, timestamp=True):
    ...
    for v in self.context.ip_versions:
        for d in metacl.DIRECTIONS:
            ...
            header, newacl_rules[v][d], footer = compile_one(
                self, name='TEMP', direction=d, ip_version=v, timestamp=timestamp)
    ...
    for router in self.context.get_router_connections():
        ...
        if not pxscp.pxscp(src_path=f.name, dst_username=router._username,
                           dst_server=router.hostname, dst_path=temp_acl_file_on_router,
                           password=router._password):
            raise Error('SCP Copy failed! Squeeze might be required on router.')

        log.debug('Uploaded %s with content: %s' %
                  (open(f.name).readlines(), temp_acl_file_on_router))
...
class Router(generic.Router):
    ...
    def __init__(self, hostname, *args, **kwargs):
        ...
        if hostname in self.__shared_states:
    ...
    def _connect(self, hostname, username, password, port=22,
        ...
        try:
            ...
            self._client.login(hostname, username, password, auto_prompt_reset=False)
        ...
        *ignore* may contain a list of error messages that will be ignored.
        ...
        try:
            ...
            if not self._client.prompt(timeout=300):
                raise ErrorTimeout("Execution of command '%s' timeouted after 5 minutes." %
                                   command)
            ...
            if input:
                for l in input:
                    ...
                    if not self._client.prompt(timeout=300):
                        raise ErrorTimeout("Execution of input '%s' timeouted after 5 minutes." %
                                           input)
            out = ''
            ...
            if len(out) > 0 and error:
                ...
                if error_lines:
                    ...
                    raise ErrorMessageRecived("Command %s produced the following error: %s" %
                                              (command, ' '.join(error_lines)))

            log.debug('From %s: %s' % (self.name, out))

            return out
        except pxssh.ExceptionPxssh, e:
            raise Error("SSH timeout while executing: '%s' on %s" % (command, self.name))
    ...
    def read_acl(self, name, ip_version='ipv4'):
        ...
        if self._running:
            ...
            response = filter(lambda l: l.startswith('permit') or l.startswith('deny'), response)
    ...
    def delete(self, path):
        ...
        try:
            ...
            self._client.sendline(command)
            i = self._client.expect([re.escape('Delete filename ['+path+']? '), pxssh.TIMEOUT,
