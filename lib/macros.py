...
class lan(Macro):
    ...
    def call(self, macl):
        if 'ipv6' in macl.context.ip_versions:
            ...
            macl.acl_out.insert(0, metacl.Rule.from_string(
                                'permit ip any fe80::/10', context=macl.context, parent=self))
            macl.acl_out.insert(1, metacl.Rule(
                                'permit', metacl.Filter(['icmp'],
                                                        macl.context.get_alias('local'),
                                                        first,
                                                        parent=self), parent=self))
            macl.acl_in.insert(0, metacl.Rule.from_string(
                               'permit ip fe80::/10 any', context=macl.context, parent=self))
            macl.acl_in.insert(1, metacl.Rule.from_string(
                               'permit ip local fe80::/10', context=macl.context, parent=self))
            macl.acl_in.insert(2, metacl.Rule(
                               'permit', metacl.Filter(['icmp'],
                                                       macl.context.get_alias('local'),
                                                       first,
                                                       parent=self), parent=self))
...
class dhcp6(Macro):
    ...
    def call(self, macl):
        ...
        macl.acl_in.insert(
            0, metacl.Rule(
                'permit', metacl.Filter(['udp'], macl.context.get_alias('local'), first,
                                        dports=metacl.Ports('546,547'), parent=self), parent=self))
...
class dhcp(Macro):
    ...
    def call(self, macl):
        ...
        if len(self.args) > 0:
            ...
            dhcp_servers += [i for i in self.args if not i in self.dhcp_classes]
        else:
...
class domain(Macro):
    ...
    dns_servers_old = ['131.188.3.72', '131.188.3.73', '131.188.2.103', '131.188.2.104']
    ...
    def call(self, macl):
        if self.args == ['old']:
            for ip in self.dns_servers_old:
                macl.acl_in.insert(0, metacl.Rule(
                    'permit', metacl.Filter(['tcp', 'udp'],
                                            macl.context.get_alias('local'),
                                            IPv4Descriptor(ip),
                                            dports=metacl.Ports('53'),
    ...
    def call(self, macl):
        if len(self.args) == 0:
            ...
            for h in self.args:
                ...
                macl.acl_in.insert(
                    0, metacl.Rule(
                        'permit', metacl.Filter(['tcp'],
                                                h,
                                                macl.context.get_alias('netbackup'),
                                                sports=metacl.Ports('13782'),
                                                parent=self),
                        extensions=['established'], parent=self))
    ...
    def call(self, macl):
        if len(self.args) == 0:
            ...
            for h in self.args:
                ...
                macl.acl_in.insert(
                    0, metacl.Rule(
                        'permit', metacl.Filter(['icmp', 'tcp', 'udp'],
                                                h,
...
class block(Macro):
    ...
    def call(self, macl):
        ...
        macl.acl_out.insert(
            0, metacl.Rule('deny', metacl.Filter(['ip'],
                                                 macl.context.get_alias('any'),
                                                 ips,
                                                 parent=self), parent=self))
